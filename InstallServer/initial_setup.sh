...
execute_remote $remote_user $remote_ip $remote_port $root_password "mkdir /home/opDevUser/.ssh && chown opDevUser:opDevUser /home/opDevUser/.ssh" $log_file
execute_remote $remote_user $remote_ip $remote_port $root_password "runuser -l opDevUser -c 'ssh-keygen -t rsa -N \"\" -f ~/.ssh/id_rsa'" $log_file
