...
global.logger = new VersatileLogger('StrategyAnalyzerServer', process.argv, true);
...
import { BacktestingController, DataController, TradingController, StrategyManagerController } from './controllers';
...
server.listen(SERVER_PORTS.STRATEGY_ANALYZER_PORT, () => {
  global.logger.info(`SA Server running on port ${SERVER_PORTS.STRATEGY_ANALYZER_PORT}`);
});

//  Init controllers
const usersNotificationsController = new UsersNotificationsController(app);
const strategyManagerController = new StrategyManagerController(app);
const dataController = new DataController(strategyManagerController);
const tradingController = new TradingController(strategyManagerController);
