...
import { extractOHLCV, createOHLCVSDataPoints } from "./helpers/ohlcvs";
...
const INFLUXDB_TOKEN = "4ZfjXEI2NKF2OlcGCPvHcC7q3emHJtD_36PqUdGMEAeeH0KxAN7Z7l5iFrQFfR7hQElnn7EwP7uCAW9olVWUHA=="
...
export class InfluxDBWrapper {
    ...
    async clearBucketData() {
        ...
        console.log("🚀 ~ file: InfluxDBWrapper.ts:36 ~ InfluxDBWrapper ~ clearBucketData ~ body:", body)
    ...
    async getPairData(pair: string) {
        ...
        console.log("🚀 ~ file: InfluxDBWrapper.ts:140 ~ InfluxDBWrapper ~ getPairData ~ pair:", pair)
        ...
        console.log("🚀 ~ file: InfluxDBWrapper.ts:145 ~ InfluxDBWrapper ~ getPairData ~ fluxQuery:", fluxQuery)
        const results: any[] = [];

        await queryApi.queryRows(fluxQuery, {
            next(row: any, tableMeta: any) {
                const o: any = tableMeta.toObject(row);
                console.log("🚀 ~ file: InfluxDBWrapper.ts:152 ~ InfluxDBWrapper ~ next ~ o:", o)
                results.push(o);
            },
            error(error: Error) {
            ...
            complete() {
                console.log("🚀 ~ file: InfluxDBWrapper.ts:154 ~ InfluxDBWrapper ~ next ~ results:", results)
