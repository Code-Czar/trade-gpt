...
export class UsersNotificationsView extends AbstractView {
  ...
  bindExpressRoutes() {
    ...
    this.expressApp.post(this.routes.loadUserNotifications, this.wrapAsync(this.loadUserNotifications.bind(this)));
    ...
    this.expressApp.get(
      this.routes.getNotificationForPairAndTimeframe,
      this.wrapAsync(this.getNotificationForPairAndTimeframe.bind(this)),
    );

    // Updaters
    this.expressApp.put(this.routes.markNotificationAsSent, this.wrapAsync(this.markNotificationAsSent.bind(this)));
    this.expressApp.put(
      this.routes.resetNotificationSentStatus,
      this.wrapAsync(this.resetNotificationSentStatus.bind(this)),
    );
  }
  ...
  async loadUserNotifications(req: Request, res: Response) {
    try {
      let notifications;
      if (req.body.loadFromFile) {
        const from = req.body.from;
        notifications = await this.usersNotificationsController.getUserNotifications(from);
      } else {
        notifications = await this.usersNotificationsController.getUserNotifications();
      }
  ...
  async saveUserNotifications(req: Request, res: Response) {
    try {
      const result = await this.usersNotificationsController.saveUserNotifications(req.body);
  ...
  async getUsersNotifications(req: Request, res: Response) {
    try {
      const notifications = await this.usersNotificationsController.getUsersNotifications();
      res.status(200).json(notifications);
    } catch (error) {
  ...
  async addNotification(req: Request, res: Response) {
    try {
      const result = await this.usersNotificationsController.addNotification(req.body);
      res.status(200).json(result);
    } catch (error) {
  ...
  async removeNotification(req: Request, res: Response) {
    try {
      const result = await this.usersNotificationsController.removeNotification(req.body);
      res.status(200).json(result);
    } catch (error) {
